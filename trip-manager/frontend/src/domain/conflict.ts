export const planningConflictReasonLabels: Record<string, string> = {
...
const planningConflictHandlingTips: Record<string, string> = {
  ...
  CLOSED_DATE: '避开关闭日或改地点',
  OPEN_HOURS: '改为地点开放时段',
  CAPACITY: '换到更大容量地点或错峰安排'
};

export const getPlanningConflictReasonLabel = (reason: unknown): string => (
  planningConflictReasonLabels[String(reason || '').trim()] || String(reason || '').trim() || '未知冲突'
);

export const getPlanningConflictHandlingTip = (reason: unknown): string => (
  planningConflictHandlingTips[String(reason || '').trim()] || '需人工核对后处理'
);

export const isPlanningConflictManualRequired = (reason: unknown): boolean => (
