// Shared domain (ESM): time-slot definitions and helpers used by the frontend (Vite/browser).
// Keep this file pure (no framework imports).

export const timeSlotKeys = /** @type {const} */ (['MORNING', 'AFTERNOON', 'EVENING']);

export const timeSlotWindows = /** @type {const} */ ({
 ...
 */
export function getTimeSlotFromStart(startTime) {
 ...
 * @returns {'MORNING'|'AFTERNOON'|'EVENING'}
 */
export function resolveTimeSlotByOverlap(startTime, endTime) {
  ...
  /** @type {'MORNING'|'AFTERNOON'|'EVENING'|null} */
  let bestSlot = null;
  let bestOverlap = -1;
  Object.entries(timeSlotWindows).forEach(([slotKey, window]) => {
    ...
    if (overlap > bestOverlap) {
      bestOverlap = overlap;
      bestSlot = /** @type {'MORNING'|'AFTERNOON'|'EVENING'} */ (slotKey);
    }
  });
 ...
 * @returns {'MORNING'|'AFTERNOON'|'EVENING'|''}
 */
export function normalizeImportedTimeSlot(value) {
 ...
 * @returns {string}
 */
export function toSqlTimeString(value) {
