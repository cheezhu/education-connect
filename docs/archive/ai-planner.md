...
这份文档描述的是“未来规划/早期草案”。截至 2026-02-08：
- 后端未挂载 `/api/ai/*` 路由
- `trip-manager/backend/src/routes/aiPlanner.js` 不存在
- 已落地的自动化只有跨团组排程（`/api/planning/export`、`/api/planning/import`），详见 `docs/planning/*`
...
- `AI_API_KEY`：必填才会真正调用模型（兼容旧名 `AI_api_key`）
