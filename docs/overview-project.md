...
## 项目定位
Education Connect 是面向研学团组的行程排期与日历管理系统，覆盖团组管理、地点资源、日历详情、行程设计器、统计与导出，并预留 AI/Copilot 辅助排期入口。

## 当前功能清单（基于代码）
- 团组管理（列表/筛选/创建/编辑/绑定行程方案）
- 行程资源（地点管理 + 行程方案管理）
- 日历详情（Calendar Detail：拖拽创建/编辑/删除活动）
- 行程设计器（多团组 7 日时间轴 + 导出/导入跨团组排程）
- 统计报表与导出（CSV/JSON）
- 基础认证与编辑锁
- 系统设置（行程设计器与 AI 配置）
...
## 当前路由入口
- `/login`：登录
- `/groups`、`/groups/v2`：团组管理（含日历详情/行程详情/每日卡片/人员信息）
- `/designer`：行程设计器
- `/locations`：行程资源
- `/statistics`：统计报表
- `/users`：用户管理
...
2) 行程设计器导出 `planning_input.json`（`ec-planning-input@2`）-> 外部排程 -> 导入 `planning_result.json`（`ec-planning-result@1`）写回 schedules/activities
